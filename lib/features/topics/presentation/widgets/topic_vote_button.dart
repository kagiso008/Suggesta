...
import '../providers/topic_vote_provider.dart';
...
class TopicVoteButton extends ConsumerStatefulWidget {
  ...
  const TopicVoteButton({super.key, required this.topic, this.compact = false});
...
class _TopicVoteButtonState extends ConsumerState<TopicVoteButton> {
  ...
  Future<void> _handleVote() async {
    ...
    try {
      final voteProvider = ref.read(topicVoteProvider.notifier);
      await voteProvider.toggleVote(widget.topic.id, widget.topic.voteCount);

      // Also update the topics provider to reflect the new vote count
      final topicsNotifier = ref.read(topicsProvider.notifier);
      await topicsNotifier.refresh();
    } catch (e) {
      setState(() {
        _hasError = true;
      });
  ...
  Widget build(BuildContext context) {
    final voteState = ref.watch(topicVoteProvider);
    final hasVoted = voteState.maybeWhen(
      data: (votedTopicIds) => votedTopicIds.contains(widget.topic.id),
      orElse: () => false,
    );

    final voteCount = widget.topic.voteCount;
